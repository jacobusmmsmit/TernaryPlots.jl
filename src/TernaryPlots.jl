...
tick_length = 0.015 # 1 = unit vector
...
for i in 0.0:0.2:1
    ...
    Plots.annotate!(
        ...
        tick_textpos[1],
        tick_textpos[2],
        Plots.text("$(round(1-i, sigdigits=2))", 7, rotation = 0),
    )
    Plots.plot!(
        p,
    ...
    Plots.plot!(
        p,
        [start_point[1], tick_end_point[1]],
        [start_point[2], tick_end_point[2]],
        colour = :black,
    )
end
...
annotate!(p, textpos[1], textpos[2], Plots.text("C (%)", 10, :dark, rotation = 45))
...
textpos = ((pos .+ pos2) ./ 2) .+ [dist_from_graph, dist_from_graph]
plot!(p, [pos[1], pos2[1]], [pos[2], pos2[2]], arrow = true, colour = :black)
annotate!(p, textpos[1], textpos[2], Plots.text("B (%)", 10, :dark, rotation = -45))
...
plot!(p, [pos[1], pos2[1]], [pos[2], pos2[2]], arrow = true, colour = :black)
annotate!(p, textpos[1], textpos[2], Plots.text("A (%)", 10, :dark, rotation = 0))

plot(p, title = "Distribution of A, B, and C in system")
