...
"""
function ternary_contour!(plot::Plots.Plot, f::Function, stepsize::Real, coord_system::String="")
    if coord_system != "ternary" && coord_system != "cartesian"
...
"""
    ternary_contour!(plot::Plots.Plot, zs::AbstractArray, stepsize::Real)
plot: plot generated by ternary_plot
z: an array of size (1/stepsize, 1/stepsize)
stepsize: determines how fine the grid used to calculate contours is, smaller is finer
"""
function ternary_contour!(plot::Plots.Plot, zs::AbstractArray, stepsize::Real)
    xs = 0:stepsize:1
    ys = 0:stepsize:1
    if size(zs) != (1 / stepsize + 1, 1 / stepsize + 1)
    ...
    Plots.contour!(plot, xs, ys, zs, lw=1, lc=:black, axes=nothing, border=:none, ticks=false, cb=false, legend=false)
    Plots.plot!(plot, Shape([(-0.5, 0), (-0.5, 10), (1.5, 10), (1.5, -0.5), (-0.5, -0.5), (-0.5, 0), (1, 0), (0.5, √3 / 2), (0, 0)]), colour=:white, linewidth=0, linealpha=0)
...
ternary_contour(plot::Plots.Plot, f::Function, stepsize::Real, coord_system::String="") = ternary_contour!(deepcopy(plot), f, stepsize, coord_system)
ternary_contour(plot::Plots.Plot, zs::AbstractArray, stepsize::Real) = ternary_contour!(deepcopy(plot), zs, stepsize)


ternary_contour!(f::Function, stepsize::Real) = ternary_contour!(ternary_plot(), f, stepsize)
ternary_contour!(zs::AbstractArray, stepsize::Real) = ternary_contour!(ternary_plot(), zs, stepsize)
ternary_contour(f::Function, stepsize::Real) = ternary_contour!(ternary_plot(), f, stepsize)
ternary_contour(zs::AbstractArray, stepsize::Real) = ternary_contour!(ternary_plot(), zs, stepsize)